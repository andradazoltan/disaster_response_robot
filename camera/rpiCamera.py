...
def findFace(l, s, f, v, a):
    ...
    (noseStart, noseEnd) = facial_locations["nose"]
    prev_open = 0
    ...
    for image in camera.capture_continuous(rawCapture, format="bgr", use_video_port = True):
        ...
        for i, face_rect in enumerate(detected_faces):
            ...
            if (centered(frame, points[noseStart:noseEnd], center)):
                print ("true")
...
"""
Returns true if face is relatively centered in the frame
"""

def centered(frame, nose, center):
    in_center = False
    for (x,y) in nose:
        pos = (x,y)
        in_center = distance.euclidean(pos, center) <= noseBuffer
    return in_center
